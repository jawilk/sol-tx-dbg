...
class PortOnly(BasePlugin):
    ...
    def lookup(self, token):
        print("TOOOOOOKEN: ", token)
        # if token has uuid + tx_hash + inst_nr
            # get random PORT
            # launch process with args: tx_hash + inst_nr (process will load saved tx from rocket server)
            # store uuid: pid
        # else (uuid only)
            # get pid from uuid match
            # get listening PORT from pid
       # return PORT
